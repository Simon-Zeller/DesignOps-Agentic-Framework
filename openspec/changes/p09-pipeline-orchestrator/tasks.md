...
- [ ] 0.1 Create feature branch: `feat/p09-pipeline-orchestrator`
- [ ] 0.2 Verify clean working tree (`git status`)
- [ ] 0.3 Confirm Python ≥3.11 is active (`python --version`)
...
- [ ] 1.1 Create `tests/test_checkpoint_manager.py` with all `CheckpointManager` test cases from tdd.md:
...
- [ ] 1.2 Create `tests/test_crew_sequencer.py` with all `CrewSequencer` test cases from tdd.md:
...
- [ ] 1.3 Create `tests/test_result_aggregator.py` with all `ResultAggregator` test cases from tdd.md:
...
- [ ] 1.4 Create `tests/test_first_publish_agent.py` with all Agent 6 test cases from tdd.md:
...
- [ ] 1.5 Create `tests/test_crew_stubs.py` with stub crew test cases from tdd.md:
  - `test_token_engine_stub_writes_required_outputs`
  - `test_all_crew_stubs_write_minimum_outputs` (parametrized over all 8 stubs)

- [ ] 1.6 Add resume and Gate 2 tests to `tests/test_cli_init.py`:
  - `test_cli_resume_from_valid_checkpoint`
  - `test_cli_resume_no_checkpoints_exits_nonzero`
  - `test_cli_gate2_approve_exits_zero`

- [ ] 1.7 Run `pytest tests/test_checkpoint_manager.py tests/test_crew_sequencer.py tests/test_result_aggregator.py tests/test_first_publish_agent.py tests/test_crew_stubs.py` — confirm all new tests **FAIL** (ImportError or AttributeError expected; anything but a pass is correct)

- [ ] 1.8 **Git checkpoint:** `git add -A && git commit -m "test: scaffold failing tests for p09-pipeline-orchestrator"`

## 2. Implementation (TDD — Green Phase)

### 2a. CheckpointManager tool

- [ ] 2.1 Create `src/daf/tools/checkpoint_manager.py`:
...
- [ ] 2.2 Run `pytest tests/test_checkpoint_manager.py` — all CheckpointManager tests **PASS**

### 2b. Crew stub modules

- [ ] 2.3 Create `src/daf/crews/__init__.py` — re-exports all 8 `create_<crew>_crew` factories

- [ ] 2.4 Create `src/daf/crews/token_engine.py`:
...
- [ ] 2.5 Create `src/daf/crews/design_to_code.py`:
...
- [ ] 2.6 Create `src/daf/crews/component_factory.py`:
...
- [ ] 2.7 Create `src/daf/crews/documentation.py`:
...
- [ ] 2.8 Create `src/daf/crews/governance.py`:
...
- [ ] 2.9 Create `src/daf/crews/ai_semantic_layer.py`:
...
- [ ] 2.12 Run `pytest tests/test_crew_stubs.py` — all stub tests **PASS**

### 2c. ResultAggregator tool

- [ ] 2.13 Create `src/daf/tools/result_aggregator.py`:
...
- [ ] 2.14 Run `pytest tests/test_result_aggregator.py` — all ResultAggregator tests **PASS**

### 2d. StatusReporter tool

- [ ] 2.15 Create `src/daf/tools/status_reporter.py`:
...
- [ ] 2.16 Create `src/daf/tools/crew_sequencer.py`:
...
- [ ] 2.17 Run `pytest tests/test_crew_sequencer.py` — all CrewSequencer tests **PASS**

### 2f. Agent 40 — Rollback Agent

- [ ] 2.18 Create `src/daf/agents/rollback.py`:
...
- [ ] 2.19 Create `src/daf/agents/first_publish.py`:
...
- [ ] 2.20 Run `pytest tests/test_first_publish_agent.py` — all Agent 6 tests **PASS**

### 2h. CLI integration

- [ ] 2.21 Modify `src/daf/cli.py`:
...
- [ ] 2.22 Run `pytest tests/test_cli_init.py` — all CLI tests (new and existing) **PASS**

- [ ] 2.23 **Git checkpoint:** `git add -A && git commit -m "feat: implement p09-pipeline-orchestrator — Agent 6, Agent 40, crew stubs, CLI wiring"`

## 3. Refactor (TDD — Refactor Phase)

- [ ] 3.1 Review `crew_sequencer.py` — extract the I/O contract table to a named constant `CREW_IO_CONTRACTS` with typed entries; no raw dicts inline
- [ ] 3.2 Review `first_publish.py` — ensure retry routing loop does not exceed 40 lines; extract `_run_phase1_retry_loop()` helper if needed
- [ ] 3.3 Confirm all stub crew modules follow the same pattern (uniform `create_<crew>_crew` signature and stub task structure); refactor any outliers
- [ ] 3.4 Verify all tests still pass after refactor (`pytest tests/test_checkpoint_manager.py tests/test_crew_sequencer.py tests/test_result_aggregator.py tests/test_first_publish_agent.py tests/test_crew_stubs.py tests/test_cli_init.py`)
- [ ] 3.5 **Git checkpoint:** `git add -A && git commit -m "refactor: clean up p09-pipeline-orchestrator"`

## 4. Integration & Quality

- [ ] 4.1 Run full linter: `ruff check src/ tests/` — zero errors
- [ ] 4.2 Run type checker: `pyright src/daf/agents/first_publish.py src/daf/agents/rollback.py src/daf/tools/crew_sequencer.py src/daf/tools/checkpoint_manager.py src/daf/tools/result_aggregator.py src/daf/tools/status_reporter.py src/daf/crews/` — zero errors
- [ ] 4.3 Fix all lint and type errors — zero warnings policy
- [ ] 4.4 Run full test suite: `pytest --cov=src/daf --cov-report=term-missing` — confirm ≥80% line coverage for new modules
- [ ] 4.5 Verify no regressions in existing tests (all green)
- [ ] 4.6 Update `scripts/bootstrap.sh` — add a stage that exercises `daf init --resume` against a pre-populated output directory with a valid Phase 1 checkpoint, verifying the resume flag is wired and exits correctly
- [ ] 4.7 **Git checkpoint:** `git add -A && git commit -m "chore: fix lint and type errors for p09-pipeline-orchestrator"` (skip if no changes)
...
- [ ] 5.4 Push feature branch (`git push origin feat/p09-pipeline-orchestrator`)

## 6. Delivery

- [ ] 6.1 All tasks above are checked
- [ ] 6.2 Merge feature branch into main (`git checkout main && git merge feat/p09-pipeline-orchestrator`)
- [ ] 6.3 Push main (`git push origin main`)
- [ ] 6.4 Delete local feature branch (`git branch -d feat/p09-pipeline-orchestrator`)
- [ ] 6.5 Delete remote feature branch (`git push origin --delete feat/p09-pipeline-orchestrator`)
