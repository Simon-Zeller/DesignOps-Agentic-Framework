...
- [ ] 0.1 Create feature branch: `feat/p08-pipeline-config`
- [ ] 0.2 Verify clean working tree (`git status`)
- [ ] 0.3 Verify existing test suite passes (`pytest tests/ -x -q`)
...
- [ ] 1.1 Create `tests/test_config_generator.py` with the following failing test stubs:
...
- [ ] 1.2 Create `tests/test_project_scaffolder.py` with the following failing test stubs:
...
- [ ] 1.3 Create `tests/test_pipeline_config_agent.py` with the following failing test stubs:
...
- [ ] 1.4 Verify all new tests FAIL (red phase confirmation — `ImportError` or `NotImplementedError` is expected)
- [ ] 1.5 **Git checkpoint:** `git add -A && git commit -m "test: scaffold failing tests for p08-pipeline-config"`

## 2. Implementation (TDD — Green Phase)

### 2a. Capability spec

- [ ] 2.1 Create `openspec/specs/pipeline-config/spec.md` — copy from `openspec/changes/p08-pipeline-config/specs/pipeline-config/spec.md` (promotes the change‐spec to the canonical specs location)

### 2b. ConfigGenerator tool

- [ ] 2.2 Create `src/daf/tools/config_generator.py`:
...
- [ ] 2.3 Create `src/daf/tools/project_scaffolder.py`:
...
- [ ] 2.4 Create `src/daf/agents/pipeline_config.py`:
...
- [ ] 2.5 Update `src/daf/tools/__init__.py` — add `ConfigGenerator` and `ProjectScaffolder` to exports
- [ ] 2.6 Update `src/daf/agents/__init__.py` — add `create_pipeline_config_agent` and `create_pipeline_config_task` to exports

- [ ] 2.7 Verify all tests PASS (green phase confirmation — `pytest tests/test_config_generator.py tests/test_project_scaffolder.py tests/test_pipeline_config_agent.py -v`)
- [ ] 2.8 **Git checkpoint:** `git add -A && git commit -m "feat: implement p08-pipeline-config (Agent 5, ConfigGenerator, ProjectScaffolder)"`

## 3. Refactor (TDD — Refactor Phase)

- [ ] 3.1 Review `config_generator.py` — extract inference rule tables as module-level constants (scope→coverage map, a11y→score map, scope→domains map) to remove inline conditionals
- [ ] 3.2 Review `project_scaffolder.py` — ensure template strings are clean and readable; consider moving templates to module-level constants
- [ ] 3.3 Review `pipeline_config.py` — verify task description is explicit about call ordering (ConfigGenerator before ProjectScaffolder); ensure expected output clearly lists all four files
- [ ] 3.4 Ensure all tests still pass after refactor (`pytest tests/test_config_generator.py tests/test_project_scaffolder.py tests/test_pipeline_config_agent.py -v`)
- [ ] 3.5 Review code against `design.md` architecture decisions — verify deterministic tool / agent boundary is maintained
- [ ] 3.6 **Git checkpoint:** `git add -A && git commit -m "refactor: clean up p08-pipeline-config inference tables and templates"`

## 4. Integration & Quality

- [ ] 4.1 Run full linter: `ruff check src/daf/tools/config_generator.py src/daf/tools/project_scaffolder.py src/daf/agents/pipeline_config.py`
- [ ] 4.2 Run type checker: `pyright src/daf/tools/config_generator.py src/daf/tools/project_scaffolder.py src/daf/agents/pipeline_config.py`
- [ ] 4.3 Fix all lint and type errors — zero warnings policy
- [ ] 4.4 Run full test suite: `pytest tests/ -x -q`
- [ ] 4.5 Verify no regressions in existing tests
- [ ] 4.6 Check coverage: `pytest tests/test_config_generator.py tests/test_project_scaffolder.py tests/test_pipeline_config_agent.py --cov=src/daf/tools/config_generator --cov=src/daf/tools/project_scaffolder --cov=src/daf/agents/pipeline_config --cov-report=term-missing` — target ≥80% line coverage
- [ ] 4.7 **Git checkpoint:** `git add -A && git commit -m "chore: fix lint and type errors for p08-pipeline-config"` (skip if no changes)
